...
const ContactList = ({ setContacts, contacts }) => {
  ...
  useEffect(()=>{
    const fetchContacts = async ()=>{
      ...
      const fetchPromise = await axios.get(`https://contact-management-backend-03fz.onrender.com/contacts${query}`)
    ...
    const handleStatusChange = async (id,status)=>{
      try {
        await axios.put(`https://contact-management-backend-03fz.onrender.com/contacts/${id}`, { status })
     ...
     const handleDelete = async (id)=>{
      if(confirm("Are you sure you want to Delete ?")){
            try {
        await axios.delete(`https://contact-management-backend-03fz.onrender.com/contacts/${id}`);
